...
enum ViewStateEnum {
    ...
    case empty(emptyMessage: String?, image:UIImage?)
    case normalLayout
}
...
final class StateFullView: UIView {
    ...
    private func updateViewState() {
        for value in viewsDict.values {
            value.shouldUpdateView(withState: state)
        }
    }
    
    func setState(_ state:ViewStateEnum) {
        self.state = state
    }
    
    func setErrorCompletion(_ errorBlock:@escaping () -> Void) {
